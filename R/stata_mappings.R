...
translate_stata_expr <- function(expr) {
  if (is.null(expr) || nchar(trimws(expr)) == 0) return(expr)
  expr <- trimws(expr)
...
is_constant_rhs <- function(expr) {
  ...
  if (grepl("^-?\\d+(\\.\\d+)?$", expr)) return(TRUE)
  # String literal: "text"
  if (grepl('^"[^"]*"$', expr)) return(TRUE)
  # Missing value
  if (expr == ".") return(TRUE)
  FALSE
}
...
parse_egen_args <- function(args, by_group = NULL, options = NULL) {
  ...
  if (length(func_match) < 3) return(NULL)
