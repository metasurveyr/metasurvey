...
Para produccion en Azure se usa [AKS](https://azure.microsoft.com/services/kubernetes-service/)
...
## Despliegue hibrido: Registro publico + Worker privado

No necesitas correr tu propio MongoDB para acceder a las recetas
compartidas. Podes apuntar tu API/Worker privado al **registro publico de
recetas de metasurvey** para que investigadores con acceso a microdatos
puedan aplicar recetas publicadas por la comunidad.

```bash
# .env para despliegue hibrido
# Usar el MongoDB publico de metasurvey (solo lectura para recetas/workflows)
...
# Indicadores locales (tu propio MongoDB o deshabilitados)
METASURVEY_ENABLE_INDICATORS=0
METASURVEY_ENABLE_WORKER=1
```
...
- **Recetas y workflows** se obtienen del registro publico -- cualquier
  receta publicada por la comunidad esta disponible para tu worker.
- **Los microdatos son privados** -- el worker carga archivos de encuesta
  desde un volumen local, nunca expuestos externamente.
- **La estimacion es local** -- el worker aplica recetas y ejecuta workflows
...
- **[Referencia de la API y Base de Datos](api-database-es.html)** --
