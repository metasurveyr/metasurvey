...
```{r load-with-recipes, eval = FALSE}
...
Consulta la vignette [API y Base de Datos](api-database.html) para instrucciones sobre cómo desplegar la API Plumber con MongoDB para tu propia organización.
...
4. **Validar antes de compartir** -- llamar a `validate()` sobre datos de ejemplo para
   asegurar que todas las dependencias existan.
5. **Versionar las recipes** -- usar `set_version()` al actualizarlas.

## Próximos Pasos

- **[Flujos de Estimación](workflows-and-estimation.html)** -- Usar `workflow()` para calcular estimaciones ponderadas a partir de los datos procesados
- **[Caso de Estudio ECH](ech-case-study.html)** -- Ver recipes en acción en un análisis real del mercado laboral
