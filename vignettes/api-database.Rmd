...
across the community. The API is deployed on Railway and used by both the
R client functions (`api_*`) and the Shiny explorer app.
...
The Swagger UI provides an interactive endpoint explorer auto-generated
by plumber. For detailed request/response schemas and MongoDB
collection documentation, refer to the sections below.
...
The R client reads the URL from `configure_api()` first, then falls back
to the `METASURVEY_API_URL` environment variable, and finally to the
production URL.
...
The token is stored in the session and used automatically for
subsequent API calls. The client refreshes tokens automatically within
5 minutes of expiry.
...
Shiny app (Profile tab) or use it directly:
...
The server automatically sets `user` from the JWT, initializes
`downloads = 0`, generates an `id` if not provided, and assigns
...
| `GET` | `/workflows/:id` | No | Get a single workflow |
...
The `/anda/variables` endpoint provides variable metadata sourced from
...
| `value_labels` | Named list of code-to-label mappings |
...
Admin access is controlled by the `METASURVEY_ADMIN_EMAIL` environment
...
| Method | Endpoint | Auth | Description |
|--------|----------|------|-------------|
| `GET` | `/health` | No | API health and MongoDB status |
...
| `reviewed_by` | string | No | Admin email who reviewed |
...
| `downloads` | number | No | Download count (default `0`) |
| `steps` | array | No | Step expressions as strings |
| `depends_on` | array | No | Required input variable names |
| `depends_on_recipes` | array | No | Recipe IDs this depends on |
...
| `value_labels` | object | No | Code-to-label mappings |
...
**Indexes:** unique compound on `(survey_type, name)`; on
`survey_type`.

## Database Setup

To set up the database for a new deployment:
...
The setup script creates all four collections and builds the indexes.
...
The Swagger UI will be available at `http://localhost:8787/__docs__/`.
...
The API is configured for Railway deployment via the `render.yaml` in
`inst/api/`. Push the repository and configure the environment variables
...
- **[Interactive Recipe Explorer](shiny-explorer.html)** -- Browse
  recipes and workflows through the Shiny web app
