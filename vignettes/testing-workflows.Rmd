...
stopifnot(all.equal(
  get_data(svy_baked)$api_diff,
  get_data(svy_baked)$api00 - get_data(svy_baked)$api99
))
```
...
```{r test-recipes}
# First develop steps on a survey, then convert to a recipe
svy_with_steps <- step_compute(svy, score_ratio = api00 / api99)
svy_with_steps <- bake_steps(svy_with_steps)
...
my_recipe <- steps_to_recipe(
  ...
  steps = get_steps(svy_with_steps)
)
...
# evaluate_cv expects a percentage value (0-100 scale)
