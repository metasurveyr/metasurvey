...
After transforming your survey data with steps and recipes, the next task is
**estimation**: computing means, totals, ratios, and their standard errors
while accounting for the complex survey design.

The `workflow()` function wraps the `survey` package estimators
(`svymean`, `svytotal`, `svyratio`, `svyby`) and returns tidy `data.table`
results with:
...
package---real stratified school data from California.
...
Estimate the population mean of the API score in 2000:
...
Estimate the total enrollment across all schools:
...
Pass several estimation calls to `workflow()` to compute them in a single
step:
...
Use `survey::svyby()` to compute estimates for subpopulations (domains):
...
The **coefficient of variation (CV)** measures estimation precision. Use
...
| 25-35%   | Poor | General trends only |
...
## RecipeWorkflow: Publishable Estimations
...
- What estimation calls were made
...
Publish the workflow so others can discover and reuse it:
...
### Finding Workflows for a Recipe

If you have a recipe and want to know what estimations have been published
...
For broader sharing, publish workflows to the metasurvey API:
...
Here is a full pipeline from raw data to publishable estimation using the
API dataset:
...
- **[Creating and Sharing Recipes](recipes.html)** -- Build reproducible transformation pipelines
