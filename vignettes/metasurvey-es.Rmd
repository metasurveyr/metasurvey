---
title: "Introducción a metasurvey"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Introducción a metasurvey}
...
Combina el paquete `survey` [@lumley2004] con un sistema de:

- **Pasos**: Transformaciones aplicadas a los datos
- **Recetas**: Colecciones reutilizables de pasos para reproducibilidad  
- **Workflows**: Estimaciones estadísticas con cálculo automático de varianza

El paquete maneja diseños de encuestas complejos incluyendo estratificación,
conglomerados y ponderadores replicados, con validación automática de
dependencias y optimización de expresiones.

### Características Principales

- **Objetos de encuesta tipo-seguro** que encapsulan datos, ponderadores y diseño muestral
- **Evaluación lazy** de transformaciones para mejor rendimiento
- **Rastreo automático de dependencias** previene errores antes de la ejecución
...
Un objeto `Survey` agrupa los microdatos con metadatos sobre ponderadores,
edición y tipo de encuesta. Comenzaremos con un ejemplo simple usando datos
simulados que imitan la estructura de la Encuesta Continua de Hogares (ECH) de Uruguay.

La ECH es una encuesta de hogares con panel rotativo realizada por el Instituto
Nacional de Estadística (INE) de Uruguay. Variables clave incluyen:
...
dt <- data.table(
  ...
  POBPCOAC   = sample(c(2, 3, 4, 5, 6), n, replace = TRUE, 
                      prob = c(0.55, 0.03, 0.02, 0.03, 0.37)),  # Condición actividad
...
La función `add_weight()` mapea etiquetas de periodicidad (ej., "annual",
"monthly") a nombres de columnas de ponderadores en los datos. Esto permite
que la misma receta funcione en diferentes ediciones de la encuesta.
...
Los pasos son **lazy por defecto**: se registran pero no se aplican hasta que
llama a `bake_steps()`. Esto le permite:
...
Use `step_compute()` para crear variables derivadas. El paquete automáticamente:
...
Puede agrupar cálculos usando el parámetro `.by` (similar a `data.table`):

```{r paso-compute-agrupado}
...
Use `step_recode()` para crear variables categóricas a partir de condiciones.
Las condiciones se evalúan **de arriba hacia abajo**, y la primera condición
...
Use `step_join()` para combinar datos de referencia externos. Esto es útil para agregar:
...
datos_departamentos <- data.table(
  dpto = 1:19,
  nombre_dpto = c("Montevideo", "Artigas", "Canelones", "Cerro Largo", 
...
Llame a `bake_steps()` para ejecutar todas las transformaciones pendientes:

```{r hornear, eval=FALSE}
...
Después de preparar los datos, use `workflow()` para calcular estimaciones.
La función envuelve los estimadores del paquete `survey` [@lumley2004] y
devuelve resultados ordenados con errores estándar y coeficientes de variación.

**Importante:** Pase el objeto de encuesta dentro de un `list()`.
...
Calcule estimaciones para subpoblaciones usando `survey::svyby()`:
...
El **coeficiente de variación (CV)** mide la confiabilidad de la estimación.
Un CV más bajo indica estimaciones más precisas. Siguiendo las pautas del
INE Uruguay [@ine2023]:
...
Use `evaluate_cv()` para clasificar la calidad de la estimación:
...
Las recetas empaquetan pasos de transformación para **reproducibilidad** y
**compartir**. Una vez que haya desarrollado un pipeline funcional, conviértalo
en una receta que pueda ser:
...
Guarde recetas como JSON para compartir y control de versiones:

```{r guardar-receta}
tf <- tempfile(fileext = ".json")
save_recipe(receta_laboral, tf)

# La receta se guarda con:
# - Todas las definiciones de pasos
# - Metadatos (autor, descripción, tema)
# - Documentación de variables de entrada/salida
# - Información de dependencias
```

Cargar recetas desde disco:

```{r cargar-receta, eval = FALSE}
receta_cargada <- read_recipe("indicadores_laborales.json")

# Aplicar a una nueva encuesta
nueva_encuesta <- Survey$new(
  data = datos_nuevos,
  edition = "2024",
  type = "ech",
  psu = NULL,
...
nueva_encuesta$add_recipe(receta_cargada)
...
# Examinar estructura de documentación
names(doc)
...
doc$pipeline  # Secuencia paso a paso de transformaciones
```
...
Ahora que comprende los fundamentos, explore temas avanzados:

- **[Recetas](recipes.html)** -- Workflows avanzados de recetas, publicación y compartir
- **[Análisis de Paneles](panel-analysis.html)** -- Paneles rotativos, PoolSurvey, análisis longitudinal
- **[Diseños Complejos](complex-designs.html)** -- Estratificación, conglomerados, ponderadores replicados
- **[Estudio de Caso ECH](ech-case-study.html)** -- Análisis completo del mundo real de la encuesta de hogares de Uruguay
- **[Workflows de Prueba](testing-workflows.html)** -- Estrategias de validación para pipelines de producción
...
- Organización Internacional del Trabajo (OIT). (2013). *Resolución sobre las estadísticas del trabajo, el empleo y la subutilización de la fuerza de trabajo*. 
  19° Conferencia Internacional de Estadísticos del Trabajo.
- INE Uruguay. Diccionario de Datos ECH. [https://www4.ine.gub.uy/Anda5/index.php/catalog/775/data-dictionary](https://www4.ine.gub.uy/Anda5/index.php/catalog/775/data-dictionary)
