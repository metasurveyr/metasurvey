...
install_if_missing <- function(pkg, github = NULL) {
  if (!requireNamespace(pkg, quietly = TRUE)) {
    if (!is.null(github)) {
      ...
      get("pak", envir = asNamespace("pak"))(github, ask = FALSE)
    } else {
      install.packages(pkg, repos = "https://cloud.r-project.org", quiet = TRUE)
    }
  }
}
install_if_missing("eph")
install_if_missing("casen", github = "pachadotdev/casen")
install_if_missing("PNADcIBGE")
install_if_missing("ipumsr")
...
```{r casen, eval = requireNamespace("casen", quietly = TRUE)}
library(casen)
...
```{r dhs, eval = requireNamespace("haven", quietly = TRUE)}
library(haven)
...
td <- tempdir()
unzip(tf, exdir = td)
dta_file <- list.files(td, pattern = "\\.DTA$", full.names = TRUE,
                       ignore.case = TRUE)
