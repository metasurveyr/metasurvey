...
## Documentación automática de recetas

Las recetas en `metasurvey` ahora pueden generar documentación automática, permitiendo:

- Ver qué variables requiere una receta
- Saber qué variables produce
- Visualizar el pipeline de transformación
- Validar compatibilidad con encuestas

### Método `doc()` - Documentación auto-generada

El método `doc()` inspecciona los pasos de una receta y genera documentación estructurada:

```{r, eval=FALSE}
# Crear una receta de ejemplo
mi_receta <- recipe(
  name = "Indicadores de Innovación",
  user = "ANII",
  svy = survey_empty(type = "eaii", edition = "2019-2021"),
  description = "Indicadores básicos de innovación empresarial",
  topic = "innovacion",
  
  step_recode(
    innovadora,
    realiza_innovacion == 1 ~ "Sí",
    .default = "No",
    comment = "Clasificación empresa innovadora"
  ),
  
  step_compute(
    tasa_innovacion = sum(innovadora == "Sí") / n() * 100,
    comment = "Porcentaje de empresas innovadoras"
  )
)
...
# - meta: metadatos de la receta (nombre, usuario, edición, topic, DOI)
# - input_variables: variables que la receta necesita del survey
# - output_variables: variables que la receta crea
# - pipeline: descripción de cada paso con tipo, inputs, outputs y comentarios
```

### Imprimir recetas - Recipe Cards

Las recetas ahora tienen un método `print()` mejorado que muestra una "tarjeta" formateada:

```{r, eval=FALSE}
print(mi_receta)
# ── Recipe: Indicadores de Innovación ──
# Author:  ANII
# Survey:  eaii / 2019-2021
# Topic:   innovacion
# Description: Indicadores básicos de innovación empresarial
#
...
#
# ── Pipeline (2 steps) ──
#   1. [recode] → innovadora  "Clasificación empresa innovadora"
#   2. [compute] → tasa_innovacion  "Porcentaje de empresas innovadoras"
#
# ── Produces (2 variables) ──
#   innovadora [categorical], tasa_innovacion [numeric]
```

### Guardar y cargar recetas completas

Las funciones `save_recipe()` y `read_recipe()` ahora preservan todos los metadatos:

```{r, eval=FALSE}
# Guardar receta con toda la metadata
save_recipe(mi_receta, "indicadores_innovacion.json")

# El JSON incluye:
# - Todos los metadatos (name, user, edition, topic, doi, id)
# - Documentación auto-generada (input_variables, output_variables, pipeline)
# - Los pasos de transformación

# Cargar receta (devuelve objeto Recipe completo)
receta_cargada <- read_recipe("indicadores_innovacion.json")

# Verificar que se cargó correctamente
print(receta_cargada)
```

### Validar compatibilidad con encuestas

Antes de aplicar una receta, se puede validar que la encuesta tenga todas las variables necesarias:

```{r, eval=FALSE}
# Validar que eaii_2019 tiene las variables requeridas
mi_receta$validate(eaii_2019)
# Retorna TRUE si es compatible
# Lanza error si faltan variables, indicando cuáles

# Aplicar la receta después de validar
eaii_con_indicadores <- eaii_2019 %>%
  bake_recipes(mi_receta)
```

### Compatibilidad con versiones anteriores

Las funciones mantienen compatibilidad con recetas guardadas en el formato antiguo:

```{r, eval=FALSE}
# Recetas antiguas (solo con steps) siguen funcionando
receta_vieja <- read_recipe("receta_formato_viejo.json")
# Retorna objeto Recipe si tiene metadata, o lista de steps si no
```
