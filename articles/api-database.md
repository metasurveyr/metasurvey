...
recipes, workflows, and variable metadata with the community. The API is
deployed on Railway and is used by both the R client functions (`api_*`)
and the Shiny exploration application.

**Production URL:** `https://metasurvey-api-production.up.railway.app`

**Interactive API documentation (Swagger UI):**
`https://metasurvey-api-production.up.railway.app/__docs__/`

The Swagger UI interface provides an interactive endpoint explorer
automatically generated by plumber. For detailed request/response
schemas and MongoDB collection documentation, see the sections below.
...
then falls back to the `METASURVEY_API_URL` environment variable, and
finally uses the production URL.
